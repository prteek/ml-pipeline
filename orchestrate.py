...
def orchestrate_training_pipeline(image_uri, bucket='hastie'):
    ...
    error_alert_code_location = session.upload_data('error_alert.py', bucket=bucket, key_prefix='notify/code')
    ...
    preprocess_step = ProcessingStep(name='preprocess', 
                                    ...
                                    inputs=[ProcessingInput(input_name='raw_data', 
                                           ...
                                           ProcessingInput(input_name='helpers', 
                                                            source=helpers,
                                                           destination='/opt/ml/processing/input')],
    ...
    eval_code_location = session.upload_data('eval.py', bucket=bucket, key_prefix='eval/code')
    test_data_location = preprocess_step.properties.ProcessingOutputConfig.Outputs['processed_data'].S3Output.S3Uri
    ...
    evaluation_step = ProcessingStep(name='model-evaluation', 
                                    processor=eval_processor,
                                    code=eval_code_location,
                                    inputs=[ProcessingInput(input_name='helpers', 
                                                            source=helpers,
                                                           destination='/opt/ml/processing/input'),
                                           ProcessingInput(input_name='test_data', 
                                           ...
                                           ProcessingInput(input_name='model_artifacts', 
                                                           ...
                                                           destination='/opt/ml/processing/input/model/artifacts')], # Redundant input only makes graphs connect more logically
    ...
    notify_bad_model = ProcessingStep(name='notify-bad-model', 
    ...
    training_pipeline = Pipeline(name='training-pipeline',
                                ...
                                steps=[fetch_data_step, preprocess_step, training_step, evaluation_step, step_cond])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image-uri', type=str, help='ECR image uri that would be used for processing jobs e.g. "434616802091.dkr.ecr.eu-west-1.amazonaws.com/hastie:latest"', default="434616802091.dkr.ecr.eu-west-1.amazonaws.com/hastie:latest")
