...
if __name__ == '__main__':
    ...
    parser.add_argument('--end-date', type=str, help='Last Date from which look back will start to fetch data for training (yyyy-mm-dd) e.g. "2021-07-21"')
    parser.add_argument('--number-of-days', type=str, help='Number of days looking back from the date for which data needs to be fetched e.g. "7"')
    ...
    date_list = list(map(lambda i: (date - timedelta(days=i)).strftime("%Y-%m-%d"), 
                                                           range(number_of_days))) # Look back from end date
    
    get_data_from_date_list = lambda date_list: list(map(lambda d: get_data_from_dynamodb(d, table_name),
                                                         date_list))
    
    get_data_recursively = make_simple_pipe([get_data_from_date_list, 
                                             unpack_nested_list])
